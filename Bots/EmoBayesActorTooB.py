...
class EmoBayesActorTooB(object):
    
    def __init__(self, params):
        print("New BayesAct (EmoBayesActorTooB) player created for a new game.")
        self.name = params[0]
        self.longName = "EmoBayesActorTooB[" + self.name + "]"
        #value of coins
        self.score = 0.0
    ...
    #learn_prop_init = [0.05, 0.15, 0.2, 0.6]  #the self is bit more scroogy, but don't know the other at all
        ...
        learn_agent = PDEmotAgentBB(N=self.num_samples,alpha_value=1.0,
...
#Game methods
    ...
    def getNextActionBiasedCoop(self):
        #Interface with BayesAct here
        #Should always be 'agent' interactions
        #print self.longName,"learn_avgs -->",str(self.learn_avgs)
    ...
    def receiveEmotMessageBasicWrapper(self, emotion):
        learn_xobserv = [State.turnnames.index("agent"),2]#[1]#[State.turnnames.index("agent"),1] #[1]
...
class PDEmotAgentBB(EmotionalAgent):
    def __init__(self,*args,**kwargs):

        super(PDEmotAgentBB, self).__init__(self,*args,**kwargs)
